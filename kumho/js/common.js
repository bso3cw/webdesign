$(document).ready(function(){
    ...
    /* 메뉴 (.header .gnb > ul > li > a)에 마우스를 오버했을 때 header에 menu_open 클래스 추가
    메뉴 (.header)에 마우스를 아웃했을때 (벗어났을때) header에 menu_open 클래스 삭제 */
    ...
    /* .header .gnb .open 을 클릭하면 header에 menu_mobile 클래스 추가
       .header .gnb .close 를 클릭하면 header에 menu_mobile 클래스 삭제 */
    ...
    /* 1차 메뉴를 클릭해서
        --> li를 클릭할 때 실행을 하면 하위 메뉴를 클릭할 때도 이벤트가 발생
            1차 메뉴를 클릭하는 것과 하위 메뉴를 클릭하는 건 구분
            이 이벤트는 1차 메뉴를 클릭할 때만 작동해야 함.
            1차 메뉴를 클릭하는 이벤트를 1차 메뉴의 a에 줘야 함.
            --> class는 a를 감싸는 li에 줘야 함.
    ...
    $('.header .gnb > ul > li > a').on('click', function(e){
        if(pcMobile == 'mobile'){
            e.preventDefault()
            /* 1차 메뉴를 클릭했을 때 a href로 페이지가 이동하는 현상을 막기 */
            $(this).parents('li').toggleClass('sub_open')
        }
    })
    
    /* .footer .family .btn_open을 클릭하면 .footer .family에 open class가 추가
        .footer .family .btn_close를 클릭하면 .footer .family에서 open class 삭제 */
